...
The gap between current hardware ($\sim$1,100 qubits) and the RSA-2048 requirement ($\sim$20 million qubits)~\cite{gidney2021} is approximately four orders of magnitude in qubit count alone. Additionally:
\begin{itemize}[leftmargin=*]
...
This contradicts the common assumption that post-quantum algorithms impose a computational penalty. Even ML-KEM-1024 (NIST Level 5, 256-bit security) is $4$--$13\times$ faster than X25519.
...
ML-KEM-768 requires transmitting 2,272 bytes vs.\ X25519's 64 bytes---a $35\times$ increase. While the size difference is significant in relative terms, the absolute overhead of $\sim$\SI{2}{\kilo\byte} per key exchange is modest in context:
\begin{itemize}[leftmargin=*]
